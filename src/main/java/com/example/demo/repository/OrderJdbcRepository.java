...
public class OrderJdbcRepository {
     ...
     * - fetchSize를 설정하여 한 번에 가져올 row 수를 제한
     * - Consumer 콜백으로 한 건씩 처리 → 메모리 효율적
     *
     * @param consumer 각 행에 대한 처리 로직
     */
    public void streamAllOrders(Consumer<OrderDto> consumer) {
        // fetchSize 설정으로 PostgreSQL 서버 사이드 커서 활성화
    ...
    /**
     * 배치 단위로 대용량 데이터 조회
     * - 지정한 batchSize만큼씩 읽어서 리스트로 반환
     * - Consumer에서 배치 단위로 처리 가능
     *
     * @param batchSize  한 번에 처리할 건수
     * @param consumer   배치 단위 처리 콜백
     */
